...
export class AllExceptionsFilter extends BaseExceptionFilter {
    ...
    setSentry(sentryKey) {
        ...
        if (host.getType() === 'http') {
            ...
            if (this.sentryKey) {
                Sentry.captureException(exception, scope => {
                    if (Object.keys(req.sentryContext.tags).length) {
                        for (let [tagName, tagValue] of Object.entries(req.sentryContext.tags)) {
                            scope.setTag(tagName, tagValue)
                        }
                    }
                    if (req.sentryContext.breadcrumbs.length) {
                        for (let bread of req.sentryContext.breadcrumbs) {
                            scope.addBreadcrumb({
                                category: bread.f,
                                message: bread.v,
            ...
            const routeArray = host.switchToRpc().getContext().internalRepr.get('route');
            responseBody.route = 'rpc:' + routeArray.length ? routeArray[0] : 'no/valid/route';
            if (this.sentryKey) {
