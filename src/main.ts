...
async function bootstrap() {
  ...
  SentryModule(app,configService.get('SENTRY'),configService.get('PRODUCTION'));
  ...
  app
      ...
      .use(cookieParser())
      .use(
          morgan(':remote-addr :remote-user ":method :url HTTP/:http-version" :status :res[content-length] ":referrer" - :response-time ms', {
            stream: {
              write: function (str) {logger.log(str, 'Входящий запрос');}
            }
          })
      )
      .use(session({
        ...
        store: new (RedisStore(session))({logErrors: true, client: redisClient}),
      }))
  ...
  await app.listen(configService.get('PORT')).then(app => logger.debug(`Сервер запущен на порту ${app.address().port}`, 'NestApplication'));
}
bootstrap();
